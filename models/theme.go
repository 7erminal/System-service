...
// Id doesn't exist
func GetThemeById(id int64) (v *Theme, err error) {
	o := orm.NewOrm()
	v = &Theme{ThemeId: id}
	if err = o.QueryTable(new(Theme)).Filter("ThemeId", id).RelatedSel().One(v); err == nil {
		return v, nil
	}
	return nil, err
}

func GetThemeByCode(code string) (v *Theme, err error) {
	o := orm.NewOrm()
	v = &Theme{ThemeCode: code}
	if err = o.QueryTable(new(Theme)).Filter("ThemeCode", code).RelatedSel().One(v); err == nil {
		return v, nil
	}
	return nil, err
}
...
func DeleteTheme(id int64) (err error) {
	o := orm.NewOrm()
	v := Theme{ThemeId: id}
	// ascertain id exists in the database
	if err = o.Read(&v); err == nil {
		var num int64
		if num, err = o.Delete(&Theme{ThemeId: id}); err == nil {
