...
type Operator struct {
	...
	OperatorCode string    `orm:"size(80)"`
...
// GetOperatorById retrieves Operator by Id. Returns error if
// Id doesn't exist
func GetOperatorByCode(code string) (v *Operator, err error) {
	o := orm.NewOrm()
	v = &Operator{OperatorName: code}
	if err = o.QueryTable(new(Operator)).Filter("OperatorName", code).RelatedSel().One(v); err == nil {
		return v, nil
	}
	return nil, err
}
